...
export default function ServicesPage() {
 ...
 key={service.id}
 id={service.id}
 className={`py-24 ${idx % 2 === 1 ? 'bg-surface/60' : ''}`}
 >
 ...
 <span className={`px-3 py-1 ${service.badgeColor} text-ink text-xs font-bold rounded-full`}>
 {service.badge}
 </span>
 </div>
 ...
 <p className="text-accent text-lg font-medium mb-6">{service.tagline}</p>
 <p className="text-muted text-base leading-relaxed mb-8">
 {service.description}
 </p>

 <div className="mb-8">
 <h3 className="text-ink font-semibold text-sm tracking-widest uppercase mb-4">
 What We Address
 </h3>
 ...
 <span
 key={i}
 className="px-3 py-1 bg-surface border border-line text-muted text-sm rounded-full"
 >
 {med}
 </span>
 ))}
 ...
 <div className="p-5 bg-surface/60 border border-line rounded-lg">
 <h3 className="text-ink font-semibold text-sm tracking-widest uppercase mb-3">
 What to Expect
 </h3>
 <p className="text-muted text-sm leading-relaxed">{service.whatToExpect}</p>
 </div>
 </div>

 {/* Right: Pricing */}
 <div className="space-y-4">
 <h3 className="text-ink font-semibold text-sm tracking-widest uppercase mb-6">
 Pricing
 </h3>
 ...
 i === 0
 ? 'bg-gradient-to-br to-[#13132a] border-accent/40'
 : 'bg-surface border-line hover:border-line'
 }`}
 >
 <div className="flex items-center justify-between mb-2">
 <span className="text-ink font-semibold">{tier.tier}</span>
 <span className="font-serif text-2xl font-bold text-accent">{tier.price}</span>
 </div>
 <p className="text-muted text-sm">{tier.detail}</p>
 </div>
 ))}
 <div className="pt-4 space-y-2">
 <Link
 href="/book"
 className="block w-full text-center px-6 py-4 bg-accent text-[color:var(--on-accent)] font-semibold rounded tracking-wide hover:bg-accent-hover transition-colors"
 >
 Book Consultation
 </Link>
 <Link
 href="/pay"
 className="block w-full text-center px-6 py-3 border border-accent text-accent font-semibold rounded tracking-wide hover:bg-accent-soft transition-colors"
 >
 Pay for Service
 </Link>
 ...
 <Link
 href="/pay"
 className="inline-block px-10 py-4 border border-accent text-accent font-bold rounded tracking-wide hover:bg-accent-soft transition-colors"
 >
 Pay Now
 </Link>
 </div>
