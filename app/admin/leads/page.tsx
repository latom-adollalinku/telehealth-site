    ...
    <div className="min-h-screen bg-[#0a0a0a] pt-28 pb-16 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-2">
          <h1 className="font-serif text-3xl font-bold text-white">Leads</h1>
          <div className="flex items-center gap-3">
            {leads.length > 0 && (
              <button
                onClick={exportCsv}
                className="text-xs text-gray-300 border border-gray-700 rounded px-3 py-1.5 hover:text-white hover:border-gray-500"
              >
                Export CSV
              </button>
            )}
        ...
        {loading ? (
          ...
          <div className="p-8 bg-[#1a1a2e] border border-[#c9a84c]/20 rounded-xl text-center">
            <p className="text-gray-300">No leads captured yet.</p>
            <p className="text-gray-500 text-sm mt-1">
              Leads from the homepage email capture will appear here.
            </p>
          </div>
        ) : (
          <div className="bg-[#1a1a2e] border border-[#c9a84c]/20 rounded-xl overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {leads.map(l => (
                  <tr key={l.id} className="border-b border-gray-800/60 hover:bg-white/[0.02]">
