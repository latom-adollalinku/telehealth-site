      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">LATOM Wellness practice management</p>
          </div>
          <button
            onClick={() => {
            ...
            className="text-gray-400 text-sm border border-gray-700 rounded px-3 py-1.5 hover:text-white hover:border-gray-500 transition-colors"
          >
            Sign out
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {cards.map(c => (
            <Link
              key={c.href}
              href={c.href}
              className="block p-6 bg-[#1a1a2e] border border-[#c9a84c]/20 rounded-xl hover:border-[#c9a84c]/60 transition-colors"
            >
              <h2 className="text-[#c9a84c] font-semibold text-lg mb-1">{c.title}</h2>
              <p className="text-gray-400 text-sm mb-3">{c.desc}</p>
              <p className="text-white text-sm font-medium">{c.stat}</p>
            </Link>
          ))}
        </div>
      </div>
