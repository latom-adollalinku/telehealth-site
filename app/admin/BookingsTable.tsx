...
export default function BookingsTable({ initialBookings, fetchError }: Props) {
 ...
 <p className="text-muted text-xs">{fetchError}</p>
 </div>
 );
 }
 ...
 <span className="text-accent/70 font-mono">latom-wellness/consultation</span> will
 appear here.
 </p>
 </div>
 );
 ...
 <div className="grid grid-cols-[1.3fr_1.2fr_1.3fr_110px_1fr_110px_1.2fr] gap-3 px-5 py-3 bg-surface border-b border-line text-xs text-muted uppercase tracking-wider">
 <span>Date &amp; Time</span>
 <span>Patient</span>
 <span>Email</span>
 <span>Phone</span>
 ...
 <div
 key={b.uid}
 className={`grid grid-cols-[1.3fr_1.2fr_1.3fr_110px_1fr_110px_1.2fr] gap-3 px-5 py-4 border-b border-accent/5 items-center text-sm ${
 isPaid ? 'bg-green-900/5' : 'hover:bg-surface/40'
 }`}
 >
 <span className="text-gray-200 text-xs">{formatDateTime(b.startTime)}</span>
 <span className="text-ink font-medium text-sm truncate">{b.attendeeName}</span>
 <span className="text-muted text-xs truncate">{b.attendeeEmail}</span>
 <span className="text-muted text-xs">{b.attendeePhone || ' - '}</span>
 <span className="text-muted text-xs truncate">{b.service}</span>
 <span>
 {isPaid ? (
 ...
 className="px-2 py-1 text-[10px] uppercase tracking-wider rounded border border-line text-accent hover:bg-accent-soft disabled:opacity-50 transition-colors"
 ...
 className="px-2 py-1 text-[10px] uppercase tracking-wider rounded border border-line text-muted hover:bg-gray-700/50 disabled:opacity-50 transition-colors"
 ...
 <div>
 <p className="text-ink font-medium text-sm">{b.attendeeName}</p>
 <p className="text-faint text-xs">{b.attendeeEmail}</p>
 {b.attendeePhone && (
 <p className="text-faint text-xs">{b.attendeePhone}</p>
 )}
 </div>
 {isPaid ? (
 <span className="inline-block px-2 py-0.5 text-xs rounded-full bg-green-500/20 text-green-300 font-medium">
 ...
 <p className="text-muted text-xs mb-1">{formatDateTime(b.startTime)}</p>
 <p className="text-muted text-xs mb-3">{b.service}</p>
 ...
 onClick={() => handleGenerateInvoice(b)}
 disabled={status === 'invoicing'}
 className="flex-1 px-2 py-1.5 text-xs rounded border border-accent/40 text-accent hover:bg-accent-soft disabled:opacity-50"
 >
 {status === 'invoicing' ? '…' : 'Invoice'}
 </button>
 <button
 onClick={() => handleSendFollowUp(b)}
 disabled={status === 'emailing' || status === 'invoicing'}
 className="flex-1 px-2 py-1.5 text-xs rounded border border-line text-muted hover:bg-gray-700/50 disabled:opacity-50"
 >
