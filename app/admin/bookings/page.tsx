...
function TierBadge({ tier }: { tier: BookingRow['tier'] }) {
  ...
  return (
    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${map[tier] ?? 'bg-gray-800 text-gray-400'}`}>
      {tier}
    </span>
  );
}
...
function StatusBadge({ status }: { status: BookingRow['payment_status'] }) {
  ...
  return (
    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${map[status] ?? 'bg-gray-800 text-gray-300'}`}>
      {label}
    </span>
  );
}
...
export default function AdminBookingsPage() {
  ...
  return (
    <main style={{ backgroundColor: '#0a0a0a' }} className="min-h-screen text-gray-300">
      {/* Hero */}
      <section className="max-w-7xl mx-auto px-4 pt-16 pb-8">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs tracking-[0.3em] text-gray-500 uppercase mb-2">Admin</p>
            <h1
            ...
            <p className="text-gray-400 text-lg">Bookings &amp; Payments</p>
          </div>
          <button
            onClick={signOut}
            className="mt-2 px-4 py-2 text-xs tracking-widest uppercase border border-gray-700 text-gray-400 hover:text-white hover:border-gray-400 transition-colors rounded"
          >
            Sign out
          </button>
        </div>
        ...
        <div className="flex gap-1 flex-wrap">
          {tabs.map((t) => (
            <button
              ...
              style={activeTab === t.key ? { backgroundColor: '#c9a84c' } : {}}
            >
              {t.label}
            </button>
          ))}
      ...
      <section className="max-w-7xl mx-auto px-4 pb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={s.label}
              className="rounded border border-gray-800 p-4"
              style={{ backgroundColor: '#111' }}
            >
              <p className="text-xs tracking-widest uppercase text-gray-500 mb-1">{s.label}</p>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-16">
        ...
        {!loading && !error && bookings.length === 0 && (
          <div className="py-20 text-center text-gray-600 text-sm tracking-widest uppercase">
          ...
          <>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs tracking-widest uppercase text-gray-600 border-b border-gray-800">
                    ...
                    <Fragment key={b.id}>
                      ...
                      >
                        ...
                        <td className="py-3 pr-4">
                          ...
                          <p className="text-gray-500 text-xs">{b.patient_email}</p>
                          <p className="text-gray-600 text-xs">{b.patient_phone}</p>
                        </td>
                        <td className="py-3 pr-4 text-gray-300 max-w-[180px]">
                          <p>{b.service}</p>
                          {b.protocol_id && (
                        ...
                        <td className="py-3">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1 text-xs rounded border border-gray-700 text-gray-400 hover:text-white hover:border-gray-400 transition-colors"
                            >
                            ...
                            <button
                              ...
                              className={
                                editingId === b.id
                                  ...
                                  : 'px-3 py-1 text-xs rounded border border-gray-700 text-gray-400 hover:text-white hover:border-gray-400 transition-colors'
                              }
                              style={
                                editingId === b.id
            ...
            <div className="md:hidden space-y-4">
              {bookings.map((b) => (
                ...
                >
                  ...
                  <p className="text-gray-300 text-sm mb-1">{b.service}</p>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-400">{fmtAmount(b.amount_cents)}</span>
                    {b.patient_state && (
                      <span className="text-gray-600 text-xs">{b.patient_state}</span>
                    )}
                  </div>
                  <div className="flex gap-2 mt-3">
                    <button
                      onClick={() => toggleExpand(b.id)}
                      className="flex-1 px-3 py-2 text-xs rounded border border-gray-700 text-gray-400 hover:text-white hover:border-gray-400 transition-colors"
                    >
                      {expandedIds.has(b.id) ? 'Close' : 'View'}
                    </button>
                    <button
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {fields.map(
        (f) =>
          f.value && (
            <div key={f.label}>
              <p className="text-xs tracking-widest uppercase text-gray-600 mb-0.5">{f.label}</p>
              <p className="text-gray-300 text-sm whitespace-pre-wrap break-words">{f.value}</p>
            </div>
          ),
      )}
    </div>
...
function InlineEditor({ editState, saving, saveError, onChange, onSave, onCancel }: InlineEditorProps) {
  const inputClass =
    'w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-amber-600';
  ...
  return (
    <div className="space-y-3 max-w-lg">
      ...
      <div>
        <label className="block text-xs text-gray-500 mb-1 uppercase tracking-widest">Notes</label>
        <textarea
          rows={3}
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={onCancel}
          disabled={saving}
          className="px-5 py-2 text-sm rounded border border-gray-700 text-gray-400 hover:text-white transition-colors disabled:opacity-50"
        >
          Cancel
        </button>
      </div>
